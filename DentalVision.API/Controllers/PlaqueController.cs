...
namespace DentalVision.API.Controllers
{
    ...
    public class PlaqueController : ControllerBase
    {
        ...
        [HttpPost("upload")]
        public async Task<IActionResult> UploadImage([FromForm] IFormFile file, [FromForm] int patientId, [FromForm] string? notes)
        {
            ...
            // Run mock image plaque mapping pipeline
            var analysisResult = await _imageProcessingService.AnalyzeDentalImageAsync(filePath, dentalImage.Id);
            ...
            var mappedAnalysis = _unitOfWork.PlaqueAnalyses.Find(a => a.Id == analysis.Id).First();
